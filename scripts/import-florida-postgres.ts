...
async function main() {
  ...
  const payload = JSON.parse(await readFile(join(process.cwd(), "data", "licenses", "fl.json"), "utf8")) as Payload;
  const records = payload.records ?? [];
  const validation = validateLicenseRecords(records, "FL");
  if (!validation.valid) throw new Error(`Florida dataset validation failed: ${validation.errors.join(" ")}`);
  const source = payload.meta?.source;
  const sourceUrl = payload.meta?.sourceUrls?.[0];
  const retrievedAt = payload.meta?.retrievedAt;
  if (!source || !sourceUrl || !retrievedAt) throw new Error("Florida dataset is missing source metadata.");
  const { createPool } = await import("../src/lib/db");
  const pool = createPool(2);
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const audit = await client.query("insert into dataset_imports (dataset, state_code, source_url, retrieved_at, record_count, status) values ($1,$2,$3,$4,$5,'started') returning id", ["licenses", "FL", sourceUrl, retrievedAt, records.length]);
    ...
    console.log(`Imported ${records.length} Florida license records into PostgreSQL.`);
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally { client.release(); await pool.end(); }
}
main().catch((error) => { console.error("Florida Postgres import failed; transaction rolled back.", error instanceof Error ? error.message : error); process.exitCode = 1; });
